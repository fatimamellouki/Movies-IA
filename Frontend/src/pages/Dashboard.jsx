    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <section className="mb-16">
          <h2 className="text-3xl font-black bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent mb-8 flex items-center gap-2">
            ✨ Pour Vous
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            {!loadingRecs && recommendations.length === 0 && (
              <div className="col-span-full text-center py-12 text-gray-400">
                ...
                <Link to="/user-ratings" className="text-yellow-400 hover:text-yellow-300">Allez noter des films →</Link>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="mb-16">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to="/movies" className="text-yellow-400 hover:text-yellow-300 font-bold">Plus →</Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {loadingPop && [...Array(4)].map((_, i)=> <MovieSkeleton key={i} />)}
            {!loadingPop && popular.slice(0, 8).map(m=> <MovieCardWithVideo key={m.movie_id} m={m} />)}
          </div>
        </section>
        ...
        <section className="mb-16">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link to="/movies" className="text-yellow-400 hover:text-yellow-300 font-bold">Tous →</Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {loadingDisc && [...Array(4)].map((_, i)=> <MovieSkeleton key={i} />)}
            {!loadingDisc && discover.slice(0, 8).map(m=> <MovieCardWithVideo key={m.movie_id} m={m} />)}
          </div>
        </section>
        ...
        <section className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-8 border border-purple-500/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <Link to="/watchlist" className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-xl hover:shadow-lg transition-all transform hover:scale-105 text-center">
              <div className="text-3xl mb-2">📋</div>
              <h4 className="font-bold">Ma Watchlist</h4>
              <p className="text-sm text-gray-300">Films à regarder</p>
            </Link>
            <Link to="/user-ratings" className="bg-gradient-to-r from-yellow-600 to-orange-600 p-6 rounded-xl hover:shadow-lg transition-all transform hover:scale-105 text-center">
              <div className="text-3xl mb-2">⭐</div>
              <h4 className="font-bold">Mes Évaluations</h4>
              <p className="text-sm text-gray-300">Films notés</p>
            </Link>
            <Link to="/profile" className="bg-gradient-to-r from-pink-600 to-red-600 p-6 rounded-xl hover:shadow-lg transition-all transform hover:scale-105 text-center">
              <div className="text-3xl mb-2">👤</div>
